  ...
  render() {
    ...
    if (this.mode === 'run') {
      ...
      if (!this.currentCategory) {
        let out = `\n\r${ansi.styles(['normal', 'cyan'])}Select a Category or Exit:\n\r`;
        ...
        out += `\n\r${ansi.styles(['reset'])}Enter a number to run a door: ${ansi.styles(['bold', 'cyan'])}`;
        this.wrapper.write(out);
      }
    }
    ...
    if (this.mode === 'monitor') {
      ...
      out += '\n\r';
      out += `${ansi.styles(['normal', 'cyan'])}E[${ansi.styles(['bold', 'cyan'])}X${ansi.styles(['normal', 'cyan'])}]it Monitor `;
      out += `${ansi.styles(['normal', 'cyan'])}[${ansi.styles(['bold', 'cyan'])}D${ansi.styles(['normal', 'cyan'])}]isconnect `;
      out += `${ansi.styles(['normal', 'cyan'])}[${ansi.styles(['bold', 'cyan'])}R${ansi.styles(['normal', 'cyan'])}]efresh\n\r`;
      out += `${ansi.styles(['reset'])}Command: ${ansi.styles(['bold', 'cyan'])}`;
      this.wrapper.write(out);
    }
  }
      ...
      if (!this.currentCategory) {
        ...
        if (index >= 0 && index < doorsInCategory.length) {
          ...
          this.wrapper.setModule(door.code, () => {
            ...
            this.wrapper.write(`\n\r${ansi.styles(['bold', 'cyan'])}Exited ${door.gameTitle} (${door.code}). Returning to Debug Menu.\n\r`);
            this.mode = 'main';
            this.inputMode = 'char';
            this.render();
          });
