...
class Door {
  ...
  render() {
    ...
    this.process.on("exit", () => {
      // Properly clean up the serial port
      if (this.client) {
        this.client.end(); // Gracefully close the serial connection
    ...
    let connectInterval = setInterval(() => {
      ...
      if (tries >= 20) {
        console.log(
          `Failed to connect to port ${this.nodePort} after 20 tries, giving up`
        );
